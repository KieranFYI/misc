...
namespace KieranFYI\Misc\Helpers;
...
class UserAgent
{
    ...
    public array $androidDevices = [
        ...
        '5.1' => [
            ...
            '[MOTOROLA |][MOTO G|MOTO G XT1068|XT1021|MOTO E XT1021|MOTO XT1580|MOTO X FORCE XT1580|MOTO '
            . 'X PLAY XT1562|MOTO XT1562|MOTO XT1575|MOTO X PURE XT1575|MOTO XT1570 MOTO X STYLE] '
    ...
    public function generate(string $userAgent = null): string
    {
        ...
        if ($userAgent == 'chrome') {
            return 'Mozilla/5.0 (' . $this->getOS($userAgent) . ') AppleWebKit/'
                ...
                . static::chromeVersion(['min' => 47, 'max' => 55])
            ...
            return 'Mozilla/5.0 (' . $this->getOS($userAgent) . ') Gecko/'
                . (random_int(1, 100) > 30 ? '20100101' : '20130401')
                . ' Firefox/' . static::firefoxVersion(['min' => 45, 'max' => 74]);
        } elseif ($userAgent == 'explorer') {
            return 'Mozilla / 5.0 (compatible; MSIE ' . ($int = random_int(7, 11)) . '.0; '
                . $this->getOS('windows') . ' Trident / '
                . ($int == 7 || $int == 8 ? '4' : ($int == 9 ? '5' : ($int == 10 ? '6' : '7'))) . '.0)';
        } elseif (in_array($userAgent, ['mobile', 'android', 'iphone', 'ipad', 'ipod'])) {
            return 'Mozilla/5.0 (' . $this->getMobileOS($userAgent) . ') AppleWebKit/'
                . (random_int(1, 100) > 50 ? random_int(533, 537) : random_int(600, 603))
                . '.' . random_int(1, 50) . ' (KHTML, like Gecko)  Chrome/'
                . static::chromeVersion(['min' => 47, 'max' => 55])
                . ' Mobile Safari/'
                . (random_int(1, 100) > 50 ? random_int(533, 537) : random_int(600, 603)) . '.' . random_int(0, 9);
        }
     ...
     * @param string|null $selectedOS
     * @return string
     * @throws Exception
     */
    public function getOS(string $selectedOS = null): string
    {
        $randomOS = [];
        if ($selectedOS === null || in_array($selectedOS, ['chrome', 'firefox', 'explorer'])) {
            $randomOS = $selectedOS === 'explorer' ?
                $this->windowsOS :
                array_merge($this->windowsOS, $this->linuxOS, $this->macOS);
        } else {
            $randomOS += $this->{$selectedOS . 'OS'};
        }
        // randomly select on operating system
        $selectedOS = rtrim($randomOS[random_int(0, count($randomOS) - 1)], ';');

        // check for spin syntax
        if (str_contains($selectedOS, '[')) {
            $selectedOS = static::processSpinSyntax($selectedOS);
        }

        // check for random number syntax
        if (str_contains($selectedOS, ':number')) {
            $selectedOS = static::processRandomNumbers($selectedOS);
        }
     ...
     * @return string
     */
    public static function processSpinSyntax(string $selectedOS): string
    {
     ...
     * @throws Exception
     */
    public static function processRandomNumbers(string $selectedOS): string
    {
    ...
    public static function chromeVersion(array $version): string
    {
        return random_int($version['min'], $version['max'])
            . '.0.' . random_int(1000, 4000) . '.' . random_int(100, 400);
    }
    ...
    /**
     * Get Mobile OS
     * @param string|null $selectedOS Can specifiy android, iphone, ipad, ipod, or null/blank for random
     * @return array|string|null
     * @throws Exception
     */
    public function getMobileOS(string $selectedOS = null): array|string|null
    {
        $selectedOS = strtolower($selectedOS);
        $availableOS = [];
        switch ($selectedOS) {
            case 'android':
                $availableOS += $this->androidOS;
                break;
            case 'iphone':
            case 'ipad':
            case 'ipod':
                $availableOS[] = $this->mobileIOS[$selectedOS];
                break;
            default:
                $availableOS = array_merge($this->androidOS, array_values($this->mobileIOS));
        }
        // select random mobile os
        $selectedOS = rtrim($availableOS[random_int(0, count($availableOS) - 1)], ';');
        if (str_contains($selectedOS, ':androidVersion:')) {
            $selectedOS = $this->processAndroidVersion($selectedOS);
        }
        if (str_contains($selectedOS, ':androidDevice:')) {
     ...
     * @return string
     */
    public function processAndroidVersion(string $selectedOS): string
    {
     ...
     * @return string
     */
    public function addAndroidDevice(string $selectedOS): string
    {
        $devices = $this->androidDevices[substr($this->androidVersion, 0, 3)];
        $device = $devices[array_rand($devices)];

        $device = static::processSpinSyntax($device);
